...
async function analyzePlaylist() {
    ...
    try {
        ...
        try {
            ...
            // Assign BPM to tracks if we have audio features
            ...
            // Display the results with BPM data
            displayResults(playlistData, tracksWithBPM, genreStats, true);
        } catch (featureError) {
            console.warn('Audio features unavailable:', featureError);
            
            // Generate genre statistics without BPM data
            const genreStats = generateGenreStats(tracksWithGenres);
            
            // Display the results without BPM data
            displayResults(playlistData, tracksWithGenres, genreStats, false);
        }
    } catch (error) {
...
// Fetch audio features (BPM/tempo) for tracks - NOTE: This API may be deprecated
async function getAudioFeatures(trackIds) {
    const accessToken = localStorage.getItem('spotify_access_token');
    const result = {};
    
    // Try to use the batch endpoint first
    try {
        ...
        console.warn('Batch audio features unavailable, trying individual requests:', batchError);
        
        // If batch endpoint fails, try individual requests for a few tracks
        // This is a fallback, but we'll limit to 20 tracks to avoid too many requests
        const sampleTrackIds = trackIds.slice(0, 20);
        ...
        for (const trackId of sampleTrackIds) {
            try {
                ...
                console.warn(`Failed to get audio features for track ${trackId}:`, individualError);
            }
        }
        
        // If we didn't get any data from individual requests either, rethrow the original error
...
function displayResults(playlistData, tracksWithGenres, genreStats, hasBPMData) {
    ...
    // Create BPM visualization if we have BPM data
    if (hasBPMData) {
        createBPMVisualization(tracksWithGenres);
    } else {
        // Show a message that BPM analysis is unavailable
        document.getElementById('bpm-container').innerHTML = `
            <h3>BPM Analysis for DJs</h3>
            <div class="api-notice">
                <p>BPM analysis is currently unavailable. Spotify's audio features API may be deprecated or temporarily offline.</p>
            </div>
        `;
    }
...
// Display track list with genres and BPM if available
...
function setupGenreFiltering(tracks) {
    ...
    // Create genre filter controls if they don't already exist
    if (!document.querySelector('.genre-filter-controls')) {
...
function createBPMVisualization(tracks) {
    ...
    // Check if we have BPM data
    const tracksWithBPM = tracks.filter(t => t.bpm);
    if (tracksWithBPM.length === 0) {
        container.innerHTML = `
            <h3>BPM Analysis for DJs</h3>
            <div class="api-notice">
                <p>BPM analysis is unavailable for this playlist. No tempo data could be retrieved.</p>
            </div>
        `;
        return;
    }
    ...
    const averageBPM = tracksWithBPMCount > 0 ? Math.round(totalBPM / tracksWithBPMCount) : 0;
    ...
    // Sort tracks by BPM
    const sortedTracks = [...tracksWithBPM].sort((a, b) => a.bpm - b.bpm);
...
function displayPlaylistStatistics(tracks, playlistData) {
    ...
    const formattedStats = {
        ...
        averageBPM: stats.averageBPM.value ? `${stats.averageBPM.value} BPM` : 'N/A',
